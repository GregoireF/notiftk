...
const NOTITK = 'https://notitk.fly.dev';
const KEY    = process.env.NOTITK_KEY;   // undefined = auth désactivée
...
const NOTITK     = 'https://notitk.fly.dev';
const KEY        = process.env.NOTITK_KEY;
...
const res = await fetch(`${NOTITK}/api/watch`, {
  ...
  body: JSON.stringify({
    username: 'ninja',
    callback_url: 'https://ton-bot.example.com/notitk-hook',
    secret: process.env.NOTITK_WEBHOOK_SECRET,  // optionnel — HMAC-SHA256
  }),
});
const { watch_id } = await res.json();
// Stocker watch_id pour pouvoir se désenregistrer plus tard

// Recevoir les notifications (côté bot — Express, Fastify, etc.)
app.post('/notitk-hook', (req, res) => {
  // Vérifier la signature si secret configuré
  const sig = req.headers['x-notitk-signature'];
  if (sig) {
    const expected = 'sha256=' + hmac.createHmac('sha256', process.env.NOTITK_WEBHOOK_SECRET)
...
**Persistance** : les webhooks survivent aux redémarrages du serveur (SQLite). Sur Fly.io, monter un volume persistent et configurer `WEBHOOKS_DB=/data/webhooks.db`.
...
```bash
# Sans auth
curl https://notitk.fly.dev/api/status/ninja

# Avec auth
curl -H "X-API-Key: sk_live_abc123" https://notitk.fly.dev/api/status/ninja
```
...
def is_live(username: str, api_key: str | None = None) -> dict:
    ...
    r = httpx.get(f"https://notitk.fly.dev/api/status/{username}", headers=headers)
    r.raise_for_status()
    return r.json()
...
$data = json_decode(file_get_contents(
    'https://notitk.fly.dev/api/status/ninja',
    false,
...
|---|---|---|
| `API_KEYS` | _(vide)_ | Clés valides séparées par des virgules |
...
app.post('/hook', express.raw({ type: 'application/json' }), (req, res) => {
  const sig = req.headers['x-notitk-signature'];
  if (!verifySignature(req.body, sig, process.env.NOTITK_SECRET)) {
...
fly launch --name notitk --region cdg --no-deploy

# 3. Créer le volume persistant pour les webhooks
fly volumes create notitk_data --size 1 --region cdg
...
# 6. Vérifier
curl https://notitk.fly.dev/health
```
...
Koyeb offre 1 service toujours actif (0,1 vCPU, 512 MB) sans carte bancaire, mais **sans stockage persistant** en tier gratuit — les webhooks sont perdus au redémarrage.
...
infra/flyio/   — stack OpenTofu : app Fly.io + volume persistant
tests/
...
L'app Fly.io et le volume persistant sont gérés via [OpenTofu](https://opentofu.org) (≥ 1.9).
...
Le `msToken` est généré par du JavaScript obfusqué côté client, mis à jour régulièrement par TikTok. Sans lui, toutes les requêtes retournent `{}`. Re-implémenter ça serait un travail à plein temps. TikTokLive est la communauté qui s'en charge — notre rôle est de maintenir `requirements.txt` à jour via Dependabot.
...
- [ ] **Gestion des clés API via endpoint** — `POST /api/keys` pour auto-provisioning sans redémarrage.
